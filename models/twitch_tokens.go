...
func (token TwitchToken) Save() error {
	db := database.GetConnection()

	token.ID = "1"

	// created_at must be reset on refresh: the expiry check is
	// created_at + expires_in, and Twitch invalidates the previous refresh
	// token on rotation, so a stale timestamp forces a refresh on every call.
	_, err := db.NamedExec(`
	INSERT INTO twitch_tokens (id,access_token, refresh_token, expires_in) VALUES (:id, :access_token, :refresh_token, :expires_in)
	ON CONFLICT (id) DO UPDATE SET access_token = :access_token, refresh_token = :refresh_token, expires_in = :expires_in, created_at = current_timestamp
	`, token)
